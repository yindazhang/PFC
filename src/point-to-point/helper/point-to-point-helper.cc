...
{
    m_queueFactory.SetTypeId("ns3::PointToPointQueue");
                                        ...
                                        bool explicitFilename)
{
    ...
    if (!stream)
    {
        ...
        // transmit queue.
        //
        Ptr<PointToPointQueue> queue = device->GetQueue();
        asciiTraceHelper.HookDefaultEnqueueSinkWithoutContext<PointToPointQueue>(queue,
                                                                             "Enqueue",
                                                                             theStream);
        asciiTraceHelper.HookDefaultDropSinkWithoutContext<PointToPointQueue>(queue, "Drop", theStream);
        asciiTraceHelper.HookDefaultDequeueSinkWithoutContext<PointToPointQueue>(queue,
                                                                             "Dequeue",
                                                                             theStream);
...
PointToPointHelper::Install(Ptr<Node> a, Ptr<Node> b)
{
    ...
    // a->AddDevice(devA);
    Ptr<PointToPointQueue> queueA = m_queueFactory.Create<PointToPointQueue>();
    devA->SetQueue(queueA);
    Ptr<PointToPointNetDevice> devB = m_deviceFactory.Create<PointToPointNetDevice>();
    devB->SetAddress(Mac48Address::Allocate());
    // b->AddDevice(devB);
    Ptr<PointToPointQueue> queueB = m_queueFactory.Create<PointToPointQueue>();
