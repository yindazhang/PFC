...
void BuildLeafSpineRoute(
	uint32_t numTors, 
    uint32_t numAggs ,
	uint32_t RATIO){

    // uint32_t numServer = numTors * numAggs * RATIO;
    uint32_t numServerperRack = numAggs * RATIO;

	for(uint32_t aggId = 0;aggId < numAggs;++aggId){
		for(uint32_t serverId = 0;serverId < servers.size();++serverId){
			aggs[aggId]->AddHostRouteTo(serverId, serverId / numServerperRack + 1);
		}
	}
	...
	for(uint32_t torId = 0;torId < numTors;++torId){
		for(uint32_t serverId = 0;serverId < servers.size();++serverId){
			uint32_t rackId = serverId / numServerperRack;
			if(rackId != torId){
				for(uint32_t aggId = 1;aggId <= numAggs;++aggId){
					tors[torId]->AddHostRouteTo(serverId, numServerperRack + aggId);
				}
			}
			else{
...
void BuildLeafSpine(
	...
	// Initilize link
	PointToPointHelper linkServerSwitch;
	linkServerSwitch.SetDeviceAttribute("DataRate", StringValue("400Gbps"));
	linkServerSwitch.SetChannelAttribute("Delay", StringValue("1us"));
	...
	BuildLeafSpineRoute(numTors, numAggs, RATIO);
}
